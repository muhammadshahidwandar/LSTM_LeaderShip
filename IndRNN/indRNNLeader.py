...
# Training Parameters
train = True
LOGDIR = './summarylayer2_RndStrd256TC3M2_2Layer_Finl512D'
# Num_Epochs = 30
#learnrate = 1e-3
starter_learning_rate = 1e-4
training_steps = 2500  # 116.
batch_size = 20  # 128
...
def lstm_rnn_cell(num_units, dropout):
  ...
  return _cell
  ########indRNN Cell
def indlstm_rnn_cell(num_units, dropout):      
  _cell = tf.nn.rnn_cell.LSTMCell(num_units,state_is_tuple = True)
  _cell = tf.contrib.rnn.DropoutWrapper(_cell, output_keep_prob = dropout)
  return _cell
...
def RNN(x, weights, biases, dropout):
    ...
    ####################################New Code
    #cell =tf.nn.rnn_cell.MultiRNNCell([rnn.BasicLSTMCell(num_hidden,  forget_bias=1.0,activation = tf.nn.relu),
                    # rnn.BasicLSTMCell(num_hidden,  forget_bias=1.0,activation = tf.nn.relu),
    #                 rnn.BasicLSTMCell(num_hidden,  forget_bias=1.0,activation = tf.nn.relu)])
...
learning_rate = tf.train.exponential_decay(starter_learning_rate, global_step,
                                           2500, 0.90, staircase=False)

optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate, beta1=0.85).minimize(loss_op, global_step=global_step)
...
with tf.Session() as sess:
    ...
    for Groups in TestVector:
        ...
        Y_test,X_test,VecLengT = LoadTestVadlid(420,Groups)
        Y_val,X_val,VecLengV = LoadTestVadlid(420,Groups2)
        Y_train,X_train = TrainDatasetLoad(420,Groups,Groups2)
        strtPont = 0
        lengt = len(X_train)
        testdrop = 0;
        traindrop = 0.30
        ...
        for step in range(1, training_steps + 1):
            ...
            if ((strtPont + batch_size) < lengt - batch_size):
                ...
                del X_train, Y_train
                random_strid = np.random.randint(420, 550, 1)
            ...
            if step % display_step == 0 or step == 1:
                ...
                if ((acc > 0.0)&(Epochs>0)):
                    ...
                    GeoMean_val = np.power((percntg_val.item(0)) * (percntg_val.item(3)),1/3)# * (percntg_val.item(8)), 1 / 3)
                    if(GeoMean_val>-1):
                        length_t = len(X_test)
