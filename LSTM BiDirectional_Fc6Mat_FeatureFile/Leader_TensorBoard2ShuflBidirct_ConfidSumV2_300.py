...
# Training Parameters
train = True
LOGDIR = './summarylayer2_RndStrd256TC3M2_2Layer_Finl300'
# Num_Epochs = 30
#learnrate = 1e-3
...
with tf.Session() as sess:
    ...
    for Groups in TestVector:
        ...
        Y_test,X_test,VecLengT = LoadTestValid(240,Groups)
        #print('Vector Length', VecLengT)
        Y_val,X_val,VecLengV =  LoadTestValid(240,Groups2)
        print('Vector Length', VecLengV)
        Y_train,X_train = TrainDatasetLoad(240,Groups,Groups2)
        strtPont = 0
        lengt = len(X_train)
        testdrop = 0;
        traindrop = 0.30
        ...
        for step in range(1, training_steps + 1):
            ...
            if ((strtPont + batch_size) < lengt - batch_size):
                ...
                del X_train, Y_train
                random_strid = np.random.randint(240, 340, 1)
            ...
            if step % display_step == 0 or step == 1:
                ...
                if ((acc > 0.35)&(Epochs>0)):
                    ...
                    if(GeoMean_val>-1):
                        ...
                        C_mat_val = np.matrix(confusion_test)
                        percntg_test = np.multiply((C_mat_val / C_mat_val.astype(np.float).sum(axis=1)), 100)
                       ##########################################################################################
                        strg1 = 'Leave one Meating Validation Group'+ str(Groups)+'\n'
                        file1 = open("MyFile.txt","a")
                        file1.writelines(strg1)
                        for line in percntg_val:
