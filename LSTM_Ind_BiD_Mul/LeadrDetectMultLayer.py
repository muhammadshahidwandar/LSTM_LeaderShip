...
# Training Parameters
train = True
LOGDIR = './summarylayer2_RndStrd256TC3M2_2Layer_Finl512Mlyr'
# Num_Epochs = 30
#learnrate = 1e-3
...
LeaveOneMeeting = [0, 13, 37, 53, 73, 93, 117, 141, 149, 173, 189, 213, 229]
...
def TrainDatasetLoad(strid, Group1,Group2):
    ...
    #X_test = [];
    for i in range(0, 232):  # 232):214  ### 1 to 232 - Last meeting leave out, for test ranging from 219-232
...
########################
weights = {
'out': tf.Variable(tf.truncated_normal([num_hidden,num_classes],mean=0,stddev=.01))
}
biases = {
'out':tf.Variable(tf.truncated_normal([num_classes],mean=0,stddev=.01))
}
...
def RNN(x, weights, biases, dropout):
    ...
    lstm_cell = rnn.MultiRNNCell([rnn.BasicLSTMCell(num_hidden), rnn.BasicLSTMCell(num_hidden)],state_is_tuple = True)
    ...
    #act = tf.matmul(states[num_layers-1][1], weights['out']) + biases['out']
        ...
        for step in range(1, training_steps + 1):
            ...
            if ((strtPont + batch_size) < lengt - batch_size):
                ...
                if(t==4):
                    t = 0;
                    del X_train, Y_train
                    random_strid = np.random.randint(350, 550, 1)
