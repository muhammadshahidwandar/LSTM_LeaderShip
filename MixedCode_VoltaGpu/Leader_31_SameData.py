...
# Training Parameters
learning_rate = 1e-5
training_steps = 7000  # 1000
batch_size = 20  # 128
display_step = 300  # accuracy and loss on training batch will be displayed on consol after these steps
...
################Test Data
Y_test2 = Train_Data[0:100, -1]
X_test = X_train[0:100]#np.array(X_test, dtype=np.float32)
Y_test1 = keras.utils.to_categorical(Y_test2, num_classes)
...
def RNN(x, weights, biases):
    ...
    dropout = tf.nn.rnn_cell.DropoutWrapper(lstm_cell, output_keep_prob=0.30)
    ######
...
with tf.Session() as sess:
    ...
    for step in range(1, training_steps + 1):
        ...
        if step % display_step == 0 or step == 1:
            ...
            print('Test Batch_y is', Y_test1[34:54, :])
            loss, acc = sess.run([loss_op, accuracy], feed_dict={X: X_test[34:54].reshape(-1, timesteps, num_input),
                                                                 Y: Y_test1[34:54, :]})
    ...
    # save_path = saver.save(sess, "/home/smuhammad/LeaderTensorflow/model",global_step = global_step)
    # print("Model saved in path: %s" % save_path)
    # coord.request_stop()
    # coord.join(threads)
    # Calculate accuracy for Leave one meeting test
    #X_test = X_train[0:100]  # X_test.reshape((-1, timesteps, num_input))
    X_test   = X_train[0:100]  #
    X_test   = X_test.reshape((100, timesteps, num_input))
    test_len = len(X_test)
    # Y_test   =  Y_test.labels[:test_len]
    ans = sess.run(prediction, feed_dict={X: X_test})
    # sess.run(accuracy, feed_dict={X: X_test, Y: Y_test}))
    Predict = np.argmax(ans, 1)
    confusion_mat = confusion_matrix(Y_test2, Predict)
    np.set_printoptions(threshold=np.nan)
    print('confusion Matrix', confusion_mat, ans, keras.utils.to_categorical(Y_test2, num_classes))
    # saver = tf.train.Saver()
    # scipy.misc.imsave('/home/smuhammad/LeaderTensorflow/outfile.jpg', np.array(confusion_mat,dtype='int32'))
    print("Testing Accuracy:", \
          sess.run(accuracy, feed_dict={X: X_test, Y: keras.utils.to_categorical(Y_test2, num_classes)}))
