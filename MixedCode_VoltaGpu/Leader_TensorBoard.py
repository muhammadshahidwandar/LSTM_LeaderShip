...
X_test = [];
#X_train = [];
for i in range(150, 230):#232):  ### 1 to 232 - Last meeting leave out, for test ranging from 219-232
    ...
    while (endframIdx <= length):
        ...
        if ((i>195)&(i<214)):  # If index is less than 219, make it a part of Training data and label
            if(check!=5):
...
# Define weights
weights = {
    'out': tf.Variable(tf.random_normal([num_hidden-128, num_classes]))
}
biases = {
...
def RNN(x, weights, biases,dropout):
    ...
    lstm_cell = rnn.MultiRNNCell([rnn.BasicLSTMCell(num_hidden), rnn.BasicLSTMCell(num_hidden-128)])
...
with tf.Session() as sess:
    ...
    for step in range(1, training_steps + 1):
        ...
        loss = sess.run([optimizer,loss_op], feed_dict={X: batch_x, Y: batch_y,dropout:traindrop})
